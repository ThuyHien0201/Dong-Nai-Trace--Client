...
import fieldBg from "@assets/image_1785567648403.png";
import {
  Eye,
  EyeOff,
  LockKeyhole,
...
function TraceMark({
  ...
  return (
    ...
    >
      <div
        className={`relative flex shrink-0 items-center justify-center ${compact ? "h-10 w-10" : "h-[52px] w-[52px]"}`}
      >
        <img
          src={skhcnLogo}
      ...
      <div className="leading-none">
        <div
          className={`font-bold tracking-[-.04em] ${onDark ? "text-white" : "text-[#2740BA]"} ${compact ? "text-[17px]" : "text-[20px]"}`}
        >
          Đồng Nai{" "}
          <span className={onDark ? "text-[#ffb265]" : "text-[#E8650A]"}>
            Trace
          </span>
        </div>
        <div
          className={`mt-1 font-mono uppercase tracking-[.16em] ${onDark ? "text-blue-100/70" : "text-slate-500"} ${compact ? "text-[8px]" : "text-[9px]"}`}
        >
          {subtitle ?? "Trace with confidence"}
        </div>
      </div>
    </div>
  );
